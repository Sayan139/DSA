...
void optimal(vector <int> arr){
    ...
    for(int i=n-1; i>ind; i--){
        if(arr[i] > arr[ind]){ //  here if know the break point the right side of it the number is in decreasing order so thats why
        // starting from total right will give you the minimum number grater then arr[i]
            swap(arr[i],arr[ind]);
            break;
        }
    }
